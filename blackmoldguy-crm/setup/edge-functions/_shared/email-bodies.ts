// =====================================================================
// Email bodies — Black Mold Guy
// Edited here, deployed with the edge functions. The CRM can also override
// any of these by writing to email_templates.body_html in Supabase.
// =====================================================================

export type TemplateKey =
  | 'welcome'
  | 'nosale_followup_1' | 'nosale_followup_2' | 'nosale_followup_3'
  | 'nosale_followup_4' | 'nosale_followup_5'
  | 'reactivation_owe_you' | 'reactivation_allergies' | 'reactivation_savings'
  | 'reactivation_environment' | 'reactivation_case_study' | 'reactivation_come_back'
  | 'review_request' | 'review_5star_google' | 'review_low_internal';
...
.foot a{color:#7a8275}
.preheader{display:none;font-size:1px;color:#f7f5f0;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden}
...
You're receiving this because you reached out about a mold concern. <a href="{{unsubscribe_url}}">Unsubscribe</a>.
</div>
</div></div></body></html>`;
...
export const TEMPLATES: Record<TemplateKey, { subject: string; html: (lead: any) => string; text?: (lead: any) => string }> = {

  // ===================================================================
  // 1. WELCOME — fires immediately on opt-in
  // ===================================================================
  welcome: {
    subject: 'Got it — your assessment request is in (Deonco)',
    html: (l) => wrap(`
<h1>${l.first_name || 'Hi'} — got your request.</h1>
...
${signoff}
`, 'I got your assessment request — here\'s what to tell me next.')
  },

  // ===================================================================
  // 2. NO-SALE FOLLOW-UP SEQUENCE (status: follow-up)
  // ===================================================================
  nosale_followup_1: {
    subject: 'Why bleach makes mold worse (yes, really)',
    html: (l) => wrap(`
<h1>One thing I should've mentioned on our call</h1>
<p>${l.first_name || 'Hey'} — Deonco here. After we talked, I realized I forgot to bring up something that matters for anyone trying to handle mold themselves first.</p>
...
${signoff}
`, 'The bleach trap — why most DIY mold fixes come back worse.')
  },

  nosale_followup_2: {
    subject: 'What people forget to ask mold companies',
    html: (l) => wrap(`
<h1>The 4 questions most people don't think to ask</h1>
<p>${l.first_name || 'Hey'} — when I do consults, I keep seeing the same pattern: homeowners get quotes from 2-3 companies and don't know what to ask. Here are the four questions that separate real operators from people who will take your money and create a bigger problem.</p>
...
${signoff}
`, '4 questions that separate honest mold companies from the rest.')
  },

  nosale_followup_3: {
    subject: 'What is actually in the solutions we use',
    html: (l) => wrap(`
<h1>Why I obsess over what we spray</h1>
<p>${l.first_name || 'Hey'} — most of the families I work with came to me because someone in the home is already sick. Chemically sensitive. Autoimmune. Kids with asthma. After what they've been through, the LAST thing they need is a second exposure from the cleanup.</p>
...
${signoff}
`, 'What\'s in our treatment products — full transparency.')
  },

  nosale_followup_4: {
    subject: 'How one family stopped getting sick (case study)',
    html: (l) => wrap(`
...
${signoff}
`, 'Real case: how we found what 2 other companies missed.')
  },

  nosale_followup_5: {
    subject: 'Why I started doing this',
    html: (l) => wrap(`
...
${signoff}
`, 'The reason this is personal — why I started The Black Mold Guy.')
  },

  // ===================================================================
  // 3. HORMOZI/CORY McCARTHY REACTIVATION SEQUENCE (manual trigger)
  // ===================================================================
  reactivation_owe_you: {
    subject: 'I owe you a free re-check (my mistake)',
    html: (l) => wrap(`
<h1>I owe you something</h1>
<p>${l.first_name || 'Hey'} — Deonco here. I was reviewing my last 12 months of work and your file came across my desk. Whether we did a full job for you or just an inspection, I realized I haven't checked in on you and that's on me.</p>
...
${signoff}
`, 'I owe you a free re-check — my mistake for not following up.')
  },

  reactivation_allergies: {
    subject: 'Quick question — are your allergies worse this year?',
    html: (l) => wrap(`
<h1>The allergy/mold connection nobody mentions</h1>
<p>${l.first_name || 'Hey'} — quick one.</p>
...
${signoff}
`, 'Allergies worse this year? It might not be allergies.')
  },

  reactivation_savings: {
    subject: 'A few hundred bucks now or $25K later',
    html: (l) => wrap(`
...
${signoff}
`, 'Inspections save 10x what they cost — the numbers from my own files.')
  },

  reactivation_environment: {
    subject: 'The one room in your house that holds mold longest',
    html: (l) => wrap(`
...
${signoff}
`, 'The 5 most-missed mold spots — none of them are obvious.')
  },

  reactivation_case_study: {
    subject: 'After 6 years, she finally got her answer',
    html: (l) => wrap(`
...
${signoff}
`, 'Six years sick, two failed companies, then we found it.')
  },

  reactivation_come_back: {
    subject: 'Still on the fence? Read this when you have 2 minutes.',
    html: (l) => wrap(`
<h1>One last note from me</h1>
<p>${l.first_name || 'Hey'} — last email in this thread, I promise.</p>
...
<p style="font-size:13.5px;color:#7a8275;margin-top:24px">If you'd rather stop hearing from me — totally fine. <a href="{{unsubscribe_url}}" style="color:#7a8275">Click here</a>.</p>
${signoff}
`, 'Last email — open offer, no pressure, you decide.')
  },
  ...
  // ===================================================================
  review_request: {
    subject: 'A quick favor (60 seconds)',
    html: (l) => wrap(`
<h1>How did I do, ${l.first_name || 'friend'}?</h1>
...
<div style="text-align:center;margin:30px 0">
  <a class="cta gold" href="{{review_url}}" style="font-size:16px;padding:16px 32px">Rate my service ★★★★★</a>
</div>
...
${signoff}
`, 'Quick favor — would you rate your service?')
  },

  review_5star_google: {
    subject: 'Thank you — would you share this on Google?',
    html: (l) => wrap(`
...
<a class="cta gold" href="{{google_review_url}}">Share on Google — takes 60 sec</a>
<p style="font-size:13.5px;color:#7a8275;margin-top:20px">If you'd rather not, no problem at all. You've already done plenty.</p>
${signoff}
`, 'Thank you for the 5 stars — would you share it on Google?')
  },

  review_low_internal: {
    subject: '[CRM] Sub-4-star review just submitted',
    html: (l) => wrap(`
...
<b>Rating:</b> {{stars}} stars<br>
<b>Feedback:</b><br>
<i>{{comment}}</i></p>
<a class="cta" href="{{crm_url}}">Open lead in CRM</a>
${signoff}
`, 'Internal alert — sub-4-star review needs follow-up.')
  },
};
